...
function initializeGame(allSentences) {
  ...
  const hintSound = new Audio('meow.mp3'); hintSound.volume = 0.08;
  const correctSound = new Audio('correct.wav'); correctSound.volume = 0.8;
  const harpSound = new Audio('harp.wav'); harpSound.volume = 0.9;
  ...
  const typeSound = new Audio('type.wav'); typeSound.volume = 0.25;
  ...
  form.addEventListener('submit', e => {
    ...
    if (isCorrect) {
      ...
      if (consecutiveCorrect === 3) {
        ...
        // Pass true to keep happy cat state
        ...
        if (typeof confetti === 'function') {
          ...
          // Add second confetti burst after a delay for longer celebration
          setTimeout(() => {
            confetti({
              particleCount: 80,
              spread: 60,
      ...
      // Improved sound handling with pre-loading and multiple attempts
      if (soundEnabled) {
        // First reset the sound
        correctSound.pause();
        correctSound.currentTime = 0;
        
        // Try to play with better error handling and retry logic
        const playPromise = correctSound.play();
        
        if (playPromise !== undefined) {
          playPromise.catch(error => {
            console.log("First attempt to play correct sound failed:", error);
            
            // Create a backup sound and try again
            setTimeout(() => {
              const backupSound = new Audio(correctSound.src);
              backupSound.volume = correctSound.volume;
              backupSound.play().catch(e => console.log("Backup sound failed too:", e));
            }, 50);
          });
        }
      }
      ...
      if (consecutiveCorrect !== 3 && typeof confetti === 'function') {
        ...
        // Add second smaller confetti burst after a delay
        setTimeout(() => {
          confetti({
            particleCount: 50,
            spread: 45,
